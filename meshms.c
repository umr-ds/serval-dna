...
enum meshms_status meshms_conversations_list(const sid_t *my_sid, const sid_t *their_sid, struct meshms_conversations **conv)
{
  ...
  if (!m)
    goto end;
  if ((their_sid == NULL || cmp_sid_t(their_sid, &SID_BROADCAST)) && meshms_failed(status = get_my_conversation_bundle(my_sid, m)))
    goto end;
  // read conversations payload
  if ((their_sid == NULL || cmp_sid_t(their_sid, &SID_BROADCAST)) && meshms_failed(status = read_known_conversations(m, their_sid, conv)))
    goto end;
